...
- **Mutual exclusion via `dsh-skin use`**: skin activation is exclusive and managed by `dsh-skin use` (the `managed` section of `~/.dsh/profiles/<profile>/cordis.patch.yml`), so skins live as `skins/` assets only and never enter `patchFrom`.
