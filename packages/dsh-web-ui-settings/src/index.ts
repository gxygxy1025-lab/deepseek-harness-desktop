/**
 * Host half of the dsh-web-ui-settings group. Mounts the compatibility
...
/** Configuration for the optional authenticated same-host proxy mode. */
export interface WebUiSettingsConfig {
  /** Exact canonical host[:port] authorities a local authenticated proxy may forward. */
  trustedProxyHosts?: string[]
  /** Environment variable that carries the proxy's shared token. */
  proxyTokenEnv?: string
}

/** Loader-visible schema; the token value itself deliberately stays out of profile config. */
export const Config: z<WebUiSettingsConfig> = z.object({
...
export function resolveProxyAccess(
  config: WebUiSettingsConfig | undefined,
  ...
  if (proxyTokenEnv.trim() === '') {
    throw new Error('web-ui-settings: proxyTokenEnv must not be empty')
  }
