...
SDK 不暴露 preload 对象、Electron、任意 IPC、文件系统、Shell、插件变更、凭据、私钥、Token 或 DSH Runtime 对象。可选工作区文件 helper 接受调用方显式提供的已注册工作区根目录和相对路径。Desktop 通过工作区注册表解析根目录、重新验证最终规范化文件后，才交给操作系统。
