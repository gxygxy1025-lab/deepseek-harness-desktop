...
test('website fallback installer matches the desktop release version', async () => {
  ...
  assert.deepEqual(await collectWebsiteErrors(html, '2.7.0'), [])
})

test('website validation rejects stale fallback installers', async () => {
  const html = (await readFile(websitePath, 'utf8')).replaceAll('2.7.0', '0.1.9')
  const errors = await collectWebsiteErrors(html, '2.7.0')
...
test('website validation rejects stale presentation versions', async () => {
  const html = (await readFile(websitePath, 'utf8'))
    .replace('<title>DeepSeek Harness Desktop 2.7.0', '<title>DeepSeek Harness Desktop 2.2.0')
    .replace('<h1>DeepSeek Harness<br>Desktop 2.7.0</h1>', '<h1>DeepSeek Harness<br>Desktop 2.1</h1>')
  const errors = await collectWebsiteErrors(html, '2.7.0')
