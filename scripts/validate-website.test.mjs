...
test('website fallback installer matches the desktop release version', async () => {
  ...
  assert.deepEqual(await collectWebsiteErrors(html, '2.1.0'), [])
})

test('website validation rejects stale fallback installers', async () => {
  const html = (await readFile(websitePath, 'utf8')).replaceAll('2.1.0', '0.1.9')
  const errors = await collectWebsiteErrors(html, '2.1.0')
