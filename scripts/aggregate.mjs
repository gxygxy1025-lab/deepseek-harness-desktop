...
// Generated content is canonical LF, but Git may materialize tracked text as
// CRLF on Windows. Compare normalized text so the consistency check remains
// about generated data rather than checkout-specific line endings.
...
for (const r of results) {
  ...
  if (CHECK) {
    const diffs = []
    if (!existsSync(patchPath)
      || normalizeLineEndings(readFileSync(patchPath, 'utf8')) !== normalizeLineEndings(r.patch)) {
      diffs.push(relative(REPO_ROOT, patchPath))
    }
    if (!existsSync(pkgPath)
      || normalizeLineEndings(readFileSync(pkgPath, 'utf8')) !== normalizeLineEndings(r.pkgJson)) {
      diffs.push(relative(REPO_ROOT, pkgPath))
    }
    if (diffs.length) {
