...
test('star prompt is accessible, animated, dependency-free, and honest about its action', () => {
  ...
  assert.equal(STAR_PROMPT_VERSION, '2.3.0')
...
test('star prompt claims only version 2.3.0 once, including concurrent calls', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'dsh-star-prompt-'))
  const path = join(directory, 'state.json')
  try {
    const store = new StarPromptStore({ path })
    assert.equal(await store.claim('2.2.0'), false)
    assert.deepEqual(await Promise.all([store.claim('2.3.0'), store.claim('2.3.0')]), [true, false])
    assert.equal(await store.claim('2.3.0'), false)
...
test('star prompt recovers a corrupt state file without showing future versions', async () => {
  ...
  try {
    ...
    const store = new StarPromptStore({ path })
    assert.equal(await store.claim('2.3.0'), true)
    assert.equal(await store.claim('2.4.0'), false)
  } finally {
    await rm(directory, { recursive: true, force: true })
  }
})
