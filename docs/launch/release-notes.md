# DeepSeek Harness Desktop 1.0.3
...
- 安装包版本更新为 `1.0.3`，同时生成匹配的 `latest.yml`、区块映射文件和 SHA-256 校验文件，便于后续自动更新和人工核验。
...
下载 `DeepSeek-Harness-Desktop-Setup-1.0.3-x64.exe`，并使用同一 GitHub Release 中的 `SHA256SUMS.txt` 校验文件。安装包只应从项目 GitHub Release 或项目官网的官方下载链接获取。自动更新使用公开 Release 元数据；如果 Release 尚未完成发布，应用会提示用户稍后重试或前往 GitHub 下载页面。
...
- Bumped the desktop release to `1.0.3` and generated matching `latest.yml`, blockmap, and SHA-256 checksum metadata for future updates and manual verification.
